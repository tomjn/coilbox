...
use coilbox_unitpatch::{
    check_fields, clone_unit, locate_edit, parse_path, patch, patch_pending, Edit, Op, Patched,
    Refusal, RefusalKind, Value,
};
...
#[test]
fn a_value_worked_out_in_the_included_file_is_refused_with_that_file() {
    ...
    assert_eq!(refusal.file, Some(game().join(SCORPION_BASEDEF)));
    let location = refusal.location.expect("a location");
    assert_eq!(
        &read(SCORPION_BASEDEF)[location.start.byte..location.end.byte],
        "humanName"
    );
    assert!(
        refusal.to_string().contains("lozscorpion_basedef.lua"),
        "{refusal}"
    );
}
...
fn the_dry_run_and_locate_name_the_included_file() {
    ...
    let refusal = answers[1].as_ref().expect_err("name is computed");
    assert_eq!(refusal.kind, RefusalKind::FieldComputed);
    assert_eq!(refusal.file, Some(game().join(BEACON_BASEDEF)));
