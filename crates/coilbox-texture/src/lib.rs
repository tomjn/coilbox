//! Turn a texture file into an `image::RgbaImage`, and an `RgbaImage` back into
//! PNG bytes.
//!
//! `.dds` is the reason this is a crate rather than a module. Spring games ship
//! most of their art block-compressed, a webview decodes none of it, and two
//! parts of coilbox now have to: the unitsync worker's build pics, header art
//! and faction emblems, and the unit builder's Blender exports, which hand a
//! decoded image to three.js and to Blender's own importers. Neither can reach
//! the other, since the worker is a sidecar binary with no library target, so
//! the decode sits here where both can depend on it.

use base64::Engine;
use image::ImageEncoder;

/// Decode a texture by file extension into an `RgbaImage`, or `None` if the
/// format isn't supported or the bytes don't decode. `.dds` is unpacked here
/// (see [`decode_dds`]), everything else goes through the `image` crate
/// (extension-driven because TGA has no magic bytes).
///
/// `.pcx` is not here. Only build pics are ever one, the decoder is the worker's
/// own, and it has no other caller.
pub fn decode(ext: &str, bytes: &[u8]) -> Option<image::RgbaImage> {
...
mod tests {
    ...
    fn decodes_png_bytes() {
        ...
        let out = decode("png", &png).expect("png should decode");
        assert_eq!((out.width(), out.height()), (2, 2));
    }

    #[test]
    fn unknown_extension_is_none() {
        assert!(decode("xyz", &[0, 1, 2]).is_none());
    }
    ...
    fn decodes_an_uncompressed_bitmask_dds() {
        ...
        let img = decode("dds", &dds).expect("uncompressed dds should decode");
    ...
    fn an_uncompressed_dds_without_alpha_is_opaque() {
        ...
        let img = decode("dds", &dds).expect("x8r8g8b8 should decode");
        assert_eq!(img.get_pixel(0, 0).0, [0x10, 0x20, 0x30, 0xff]);
    }
    ...
    fn narrow_channels_scale_to_the_full_range() {
        ...
        let img = decode("dds", &dds).expect("r5g6b5 should decode");
