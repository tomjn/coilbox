...
//! own the value shapes. Replaces the old structured config: storage is now
//! frame-owned, this is just durable bytes.
...
mod tests {
    ...
    fn missing_file_is_empty() {
        let p = std::env::temp_dir().join("uberstress_settings_does_not_exist_xyz.json");
    ...
    fn roundtrips_opaque_string_values() {
        let dir = std::env::temp_dir().join("uberstress_settings_test");
