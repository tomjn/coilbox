...
- A condition may carry `negate = true`, and then it holds when the type underneath it does not ([#2422](https://github.com/tomjn/coilbox/issues/2422)). That is how a mission asks whether something has *not* happened, which is what a bonus objective for finishing without building a fusion plant is made of. A flag rather than a table of opposite types, because "has not built one" is the question `unit_built` already asks read the other way round, and a second table would have to grow every time the first one did.
- A negated condition is polled, whatever the type underneath it watches. Its events are the moments the answer becomes no: "has not built a factory", woken only by a unit being finished, is a question that can only ever answer no. Only a real answer is turned over, so a condition nothing implements and one that raised are false either way round, and a broken trigger does not start firing on the first frame instead of never.
...
- `build_unit` tells one of the mission's units to build something ([#2422](https://github.com/tomjn/coilbox/issues/2422)). Its builder is named the way a `guard` or an `attack` target is, out of the same table, so an actor, a named base building and a group all work. A build order is the negative of the unit def id, which is the command a player's own build click gives. With a position it is a site on the map, put through `Spring.Pos2BuildPos` the way every other building the runtime places is. With no position it is the same order given to a factory, which queues a unit in one: a factory has nowhere to put a building, so where is the whole difference between the two. Each of `count` is a separate order, because the engine reads the shift and control keys on a build order as "five of these" and "twenty of these". Every one of them is queued rather than replacing, so a mission adding a building to a builder's list does not cancel the road it is halfway through laying.
...
A team's bank, what it is paid and how much it can hold, all three of them movable mid-mission ([#2422](https://github.com/tomjn/coilbox/issues/2422)). `coilbox_economy.lua` owns them, including the per-frame payment of the income the scenario itself stated, which used to live in the gadget. Two copies of that number would be a `set_income` that appears to do nothing.

- `give_resources` moves a bank once. Positive is a gift and negative a drain, and every field is an amount, so a mission can hand over what a var holds.
- `set_income` says what a team is paid per second from now on, replacing whatever the scenario or an earlier trigger set. Negative is a continuous bleed. A resource the action does not name is left where it was.
- `give_storage` moves how much a team can hold, which is the ceiling every gift is clamped to. A gift of storage rather than a statement of it, so a mission handing the player a bigger tank does not also decide what the rest of their tanks were worth. Shrinking storage spills the excess, because the engine clamps what is held down to the new ceiling itself.

A gift goes through `Spring.AddTeamResource`, which is what a resource arriving looks like to the engine and to the post-game graph. A drain cannot: that call clamps its amount at zero, so it can only ever hand resources out. So a drain is the bank read back with `Spring.GetTeamResources` and written down with `Spring.SetTeamResource`. Not `Spring.UseTeamResource`, which refuses outright when there is less there than was asked for: a mission taking 1000 energy off a team holding 400 means take what they have, not leave them the 400. The price is that neither counts as anything a unit spent, which is what a drain the mission decided on is.

```lua
GG.CoilboxMission.economy.give("player", { metal = 500, energy = -50 })
GG.CoilboxMission.economy.setIncome("player", { energy = -5 })
GG.CoilboxMission.economy.giveStorage("player", { metal = 1000 })
GG.CoilboxMission.economy.income("player")   -- what it is paid per second now
```

## Calling the game's own Lua

`call_lua` calls a function the game already has, by dotted name ([#2422](https://github.com/tomjn/coilbox/issues/2422)). A name and not a body: a compiled mission is data read out of an archive, and a function written into a scenario would be source arriving from wherever that scenario came from and running in synced Lua on every machine in the game. Naming one the game shipped keeps what runs the game's, which is the same line `missions/extensions.lua` draws.

The name is walked from the gadget's own environment, so `GG.MyGame.StartTheStorm` reaches what a game's gadget put on `GG`, and nothing has to be listed here in advance. The function is called with the trigger's own parameters and the shared context. A name that points at nothing, or at something that is not a function, is reported once and does nothing. Whatever the function itself raises is caught by the engine's pcall around every action, so a game's mistake is a reported action rather than a callin that took the game with it.

```lua
GG.CoilboxMission.call.call("GG.MyGame.StartTheStorm", params, ctx)
```
...
- Whether it holds is the game's rule rather than the runtime's, so it is checked rather than assumed. A game's own emptiness check may discount defs: Splinter Faction's `game_end.lua` discounts anything carrying `customParams.decoration`, and an anchor built from such a def would count for nothing. Nothing the runtime can read says which defs a game discounts, and picking around every convention a game might have is guessing. So a mission team the game kills while the mission is still running is reported as an error, naming the def the team was anchored with and whether that anchor was still standing ([#933](https://github.com/tomjn/coilbox/issues/933)). The mission has already ended wrongly by then. What the report buys is that it reads as the anchor not holding rather than as the runtime not working.
...
`mission_trigger_test.lua` runs the scenario fixtures in `src/scenario/fixtures/missions/`, which are the files coilbox's own compiler emits. The runtime is proved against the emitted shape rather than against one written to suit it. `foundry_test.lua` does the same for everything runtime 7 added: the three economy actions, `build_unit` at a site and into a factory queue, `call_lua` against a function a game shipped and against a name it has not, and a bonus objective a negated condition decides both ways.
