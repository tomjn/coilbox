...
A team's bank, what it is paid and how much it can hold, all three of them movable mid-mission. `coilbox_economy.lua` owns them, including the per-frame payment of the income the scenario itself stated, which used to live in the gadget. Two copies of that number would be a `set_income` that appears to do nothing.
...
`call_lua` calls a function the game already has, by dotted name. A name and not a body: a compiled mission is data read out of an archive, and a function written into a scenario would be source arriving from wherever that scenario came from and running in synced Lua on every machine in the game. Naming one the game shipped keeps what runs the game's, which is the same line `missions/extensions.lua` draws.
