...
/**
 * The collision file for this unit, or null when no piece overrides anything.
 *
 * Null is not the same as an empty file: it means the export should take an
 * earlier one away, since a stale file left behind would keep applying volumes
 * the unit no longer asks for.
 *
...
export function buildPieceCollisionScript(
  ...
  const out: string[] = [
    ...
    "-- Pulled in by the unit script with:",
