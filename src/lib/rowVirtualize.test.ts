...
import { scrollTopForRow, visibleRowWindow } from "./rowVirtualize";

describe("visibleRowWindow", () => {
  it("renders every line when the container has not been measured", () => {
    expect(visibleRowWindow(0, 0, 5000, 18, 15)).toEqual({
      start: 0,
      end: 5000,
    });
  });
  ...
  it("windows to the scrolled range plus overscan", () => {
    ...
    expect(visibleRowWindow(900, 400, 5000, 18, 15)).toEqual({
      start: 35,
      end: 88,
    });
  });
  ...
  it("clamps the start at 0 near the top", () => {
    expect(visibleRowWindow(0, 400, 5000, 18, 15)).toEqual({
      start: 0,
      end: 38,
    });
  });
  ...
  it("clamps the end at the line count near the bottom", () => {
    ...
    expect(visibleRowWindow(scrollTop, 400, lineCount, 18, 15)).toEqual({
      start: 62,
      end: 100,
    });
  });
  ...
  it("never returns an end before start for a document shorter than the overscan", () => {
    expect(visibleRowWindow(0, 400, 3, 18, 15)).toEqual({ start: 0, end: 3 });
  });
});
...
describe("scrollTopForRow", () => {
  it("centres the target line in the viewport", () => {
    ...
    expect(scrollTopForRow(100, 5000, 400, 18)).toBe(1800 - 200 + 9);
  });

  it("clamps to the top when the target line is near the start", () => {
    expect(scrollTopForRow(0, 5000, 400, 18)).toBe(0);
  });

  it("clamps to the bottom when the target line is near the end", () => {
    const lineCount = 100;
    expect(scrollTopForRow(99, lineCount, 400, 18)).toBe(lineCount * 18 - 400);
  });
});
