  ...
  return (
    <div className="space-y-6">
      <section className="space-y-4">
        ...
        />
        <PathField label="Metal map (-m)" hint="grayscale image; brightness = metal density" value={value.metalmap} onChange={(v) => set({ metalmap: v })} disabled={disabled} filters={IMAGE_FILTERS} defaultPath={defaultPath} />
        <PathField label="Type map (-z)" hint="grayscale image; terrain-type index per pixel" value={value.typemap} onChange={(v) => set({ typemap: v })} disabled={disabled} filters={IMAGE_FILTERS} defaultPath={defaultPath} />
        <PathField label="Minimap (-minimap)" hint="image (PNG/TGA/BMP); optional preview" value={value.minimap} onChange={(v) => set({ minimap: v })} disabled={disabled} filters={IMAGE_FILTERS} defaultPath={defaultPath} />
        <PathField label="Vegetation map (-v)" hint="grayscale image; vegetation density" value={value.vegmap} onChange={(v) => set({ vegmap: v })} disabled={disabled} filters={IMAGE_FILTERS} defaultPath={defaultPath} />
        <PathField
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Min height (-minh)" hint="height at black; default 0">
            <Input type="number" value={value.minh} onChange={(e) => set({ minh: e.target.value })} disabled={disabled} placeholder="0" />
          </Field>
          <Field label="Max height (-maxh)" hint="height at white; default 1">
            <Input type="number" value={value.maxh} onChange={(e) => set({ maxh: e.target.value })} disabled={disabled} placeholder="1" />
          </Field>
        </div>
        <CheckField
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Threshold (-th)" hint="tile match tolerance; default 0.8">
            <Input type="number" step="0.1" value={value.th} onChange={(e) => set({ th: e.target.value })} disabled={disabled} placeholder="0.8" />
          </Field>
          <Field label="Compare count (-ccount)" hint="tiles to compare in modes 2 & 4; default 64">
            <Input type="number" min={1} value={value.ccount} onChange={(e) => set({ ccount: e.target.value })} disabled={disabled} placeholder="64" />
          </Field>
        </div>
        <CheckField
