...
describe("joinBlockedReason", () => {
  ...
  it("says to leave a room already connected to, and names it", () => {
    const reason = joinBlockedReason("Tom@127.0.0.1:8200", false);
  ...
  it("says the same of somebody else's room, which is not a loopback key", () => {
    expect(joinBlockedReason("Tom@192.168.1.45:8200", false)).toContain(
      "in a room already",
    );
  });
  ...
  it("no longer says coilbox holds one lobby connection", () => {
    for (const reason of [
      joinBlockedReason("Tom@127.0.0.1:8200", true),
      joinBlockedReason("Tom@192.168.1.45:8200", false),
    ]) {
