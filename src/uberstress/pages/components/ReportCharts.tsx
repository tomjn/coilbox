...
export function LatencyBars({ report }: { report: Report }) {
  const data = report.commands.map((c) => ({
    command: c.command,
    p50: c.p50_ms,
    p95: c.p95_ms,
    p99: c.p99_ms,
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <BarChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
        ...
        <Tooltip
          formatter={(v) =>
            typeof v === "number" ? `${v.toFixed(2)} ms` : String(v ?? "")
          }
...
export function CompareBars({
  ...
  for (const c of b.commands)
    if (!names.includes(c.command)) names.push(c.command);
  const p99 = (r: Report, cmd: string) =>
    r.commands.find((c) => c.command === cmd)?.p99_ms ?? 0;
  const data = names.map((cmd) => ({
    command: cmd,
    a: p99(a, cmd),
    b: p99(b, cmd),
  }));
...
export function TrendChart({ runs }: { runs: ReportSummary[] }) {
  ...
  const data = [...runs].reverse().map((r) => ({
    when: shortTime(r.startedAt),
    login: r.loginP99Ms ?? null,
    ping: r.pingP99Ms ?? null,
  }));
