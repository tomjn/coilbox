...
describe("a row the game's post files change", () => {
  ...
  it("says what the game does to its own value", () => {
    ...
    expect(
      screen.getByText(
        "The game changes this field as it loads. Its files say 0.1 and it loads as 0.009. It may change a value typed here too. A mutator archive and an edit-in-place write each get a value the game turns into the typed one, where loading the game proves it. Beyond All Reason's tweak slots write it as typed.",
      ),
    ).toBeTruthy();
  });
  ...
  it("says when the game sets a field its own files leave unset", () => {
    ...
    expect(
      screen.getByText(
        "The game sets this field as it loads, to 0.3. Its own files leave it unset. It may change a value typed here too. A mutator archive and an edit-in-place write each get a value the game turns into the typed one, where loading the game proves it. Beyond All Reason's tweak slots write it as typed.",
      ),
    ).toBeTruthy();
  });
