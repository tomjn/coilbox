/**
 * Packing a project across a game's numbered tweak slots, for a player who
 * does not control the host's own lobby (issue #1277).
 *
 * `tweakdefs`/`tweakunits` mod options predate Beyond All Reason, and any
 * game that declares them the same way can take a project this way. A
 * mutator archive (`package.ts`) cannot be loaded into somebody else's lobby,
 * and writing straight into a start script (`localTweakSlot.ts`) only works
 * for a local skirmish this machine is hosting. The route left for somebody
 * joining somebody else's lobby is asking the server to set a mod option
 * through the `!bset` chat command a SPADS-based autohost answers to, one
 * per numbered slot, because a real project does not fit in the one bare
 * `tweakdefs` option `localTweakSlot.ts` writes.
 *
 * The packing itself, minifying, ordering and slicing chunks to fit the
 * per-slot character cap a lobby server enforces, happens in Rust
 * (`tweak_pack.rs`): the arithmetic is shared with `compile.rs`'s own chunk
 * list and is exactly the kind of thing this project already tests in Rust
 * rather than in TypeScript (see that module's own doc comment). This file
 * only wraps the command and compares what it needed against what the
 * selected game actually declares, which is a fact `deliveryRoutes.ts`'s
 * `tweakSlotCounts` already reads live off the game rather than a number the
 * packer could know on its own.
 */
...
export interface TweakSlotPack {
  ...
  oversized: string[];
  /** A chunk that would have fit a slot on its own, but every slot the game
   *  exposes was already spoken for by chunks compiled ahead of it. */
  unplaced: string[];
}

/**
 * Compile a project, check it, and pack it across the game's numbered tweak
 ...
 */
export const workshopPackTweakSlots = defineCommand<
  {
    project: ModProject;
