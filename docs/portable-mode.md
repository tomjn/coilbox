...
Portable mode is the way to run Coilbox **self-contained**: all of its settings, caches and downloads live in a single `.coilbox` folder next to the app, instead of in the usual hidden per-user locations. It's the foundation everything else in this folder builds on — a [distribution profile](distribution-profile.md) and [bundled campaigns](campaigns.md) both live inside `.coilbox`.

There are two audiences for this, and it's worth knowing which one you are:

- **You, running Coilbox for yourself** (e.g. trying it out next to skylobby). Portable mode keeps Coilbox from scattering files across your system and lets you delete it cleanly later — just remove the folder.
- **A game author shipping Coilbox to players.** Portable mode is what makes a drop-in package possible: you hand players a folder they can run without an installer, and it can't disturb any other Spring/Recoil tool they already have.
...
In **development** (`bun tauri dev`) the binary runs from `target/debug/`, so the folder is `target/debug/.coilbox/`.

> The leading dot matters: the folder must be exactly `.coilbox`. On Windows, Explorer will let you create it if you type `.coilbox` with the trailing dot as well (`.coilbox.`) and it'll drop the trailing one; or create it from a terminal with `mkdir .coilbox`.

## What lives where

Once portable mode is on, Coilbox writes **its own** data and caches inside `.coilbox`, and you place any distribution files (profile, campaigns, media) directly in there too:
...
> Why the isolation works: a normal Coilbox install and a portable one would otherwise share the same per-user storage (they're the same app, same id), and stomp on each other. The `.coilbox` folder gives the portable copy its own private storage, sidestepping that entirely.

Sharing a content folder is read-mostly and safe, but note the two tools are independent: a download you trigger in Coilbox lands in that shared folder and skylobby will see it too, and vice versa. That's usually what you want.
...
- **Absolute** — a fixed path like `/home/you/.spring`. Fine for your own use; useless in a package you ship, because the player's paths differ.
- **Portable (relative)** — stored relative to the app folder, so it keeps working no matter where the player unzips the package or what they rename the parent folder to.
...
- A **Portable root must live inside the app folder.** Coilbox refuses to store a portable root for a folder outside the package — there'd be nothing stable to make it relative to.
- In portable mode, roots you add that happen to sit inside the app folder are relativised automatically, so they travel with the package even if you forget to tick the box.
- Everything downstream (scanning, launching, pr-downloader) receives the resolved absolute path, so bundled content behaves exactly like a normal install once found.

## Keeping a bundled game updated from GitHub

Bundling and GitHub-release updates are **complementary, not an either/or** — the intended setup for a self-updating package is to do both. You ship the current game archive inside the package (so first run works offline), and you also set the profile's [`release`](distribution-profile.md#release-object) field so Coilbox pulls newer versions from your repo's GitHub releases.

How the update check decides whether to prompt:

- On launch, Coilbox fetches your repo's **latest** release and looks at its `.sdz`/`.sd7` assets.
- It considers you up to date if **any** of those asset filenames matches a game archive already installed in your content roots (matched by **filename**, case-insensitive).
- So a bundled archive whose filename equals the latest release's asset name reads as "installed" → no update prompt. When you publish a **new** release with a **new** archive filename, the installed one no longer matches → a **Game update** badge and settings section offer a one-click download.

An update downloads the new archive into the Downloads write root's `games/` folder, rescans content so it appears without a restart, and — if the release also ships an asset named exactly `profile.json` — drops that into `.coilbox/` and offers a restart to apply the refreshed profile.

Two things to get right for this to behave:

- **Version your archive filenames.** `splinterfaction_1.3.sdz` → `splinterfaction_1.4.sdz` gives you "same version = matched, new version = update" for free. A filename that never changes (`splinterfaction.sdz`) would always look installed, so no update ever prompts.
- **Point the Downloads write root at a portable location** (Settings > Downloads) if you want updates to land inside the package rather than in a per-user folder. Otherwise the bundled archive and any downloaded update end up in different places.

If instead you'd rather ship a thin package and have players fetch the game on first run, skip bundling and rely on `release` alone, or leave the Downloads pages visible so they can grab it themselves.

## Packaging checklist

To ship a branded, self-contained Coilbox for your game:

1. **Get a Coilbox build** for each OS you're targeting (Windows `.exe`, Linux binary/AppImage, macOS `.app`). Put each in its own distribution folder.
...
> Coilbox has no installer requirement in this mode — the folder *is* the install. That's the whole point: a player can keep it on a USB stick, delete it cleanly, or run it next to whatever else they already have.

## Verifying it's active

- **Settings > Distribution profile** tells you whether a profile is loaded and where from. (If you added `profile.json` but it says "No distribution profile loaded", the `.coilbox` folder or the file name/location is wrong.)
- **Settings > Content Folders** shows a **Portable** badge next to any root stored as a relative path — confirmation your bundled content will travel.
- The simplest end-to-end test: **move the whole folder somewhere else and relaunch.** If your settings, profile and bundled content are all still there, the package is genuinely portable.
