...
Run: `cargo test -p tauri-plugin-coilbox-content standard_root_path` Expected: FAIL — `cannot find function standard_root_path`.
...
Run: `cargo test -p tauri-plugin-coilbox-content standard_root_path` Expected: PASS.
...
Run: `cargo build -p tauri-plugin-coilbox-content` Expected: builds clean; `permissions/autogenerated/commands/content_create_standard_root.toml` now exists.

- [ ] **Step 5: Run existing content tests (no regressions)**

Run: `cargo test -p tauri-plugin-coilbox-content` Expected: all pass.
...
Run: `bunx vitest run src/content/config.test.ts` Expected: FAIL — `deriveSetup` is not exported.
...
Run: `bunx vitest run src/content/config.test.ts` Expected: PASS.
...
Run: `bun run typecheck` then `bunx biome ci .` Expected: clean.
...
Run: `bun run typecheck` then `bunx biome ci .` Expected: clean (biome may reorder imports — run `bunx biome check --write src/content/pages/components/SetupCard.tsx` if so).
