...
but the user can turn it off and can click (or press a key) to dismiss it early.
...
A user preference to silence the splash. Key `splash.enabled`, boolean, default
`true` (unset == on).

- `useSplashSetting()` helper in `src/general/` (or `profile.ts`) wrapping
  `useSetting<boolean>("splash.enabled", true)`, mirroring `useFullscreenSetting`.
- A `Switch` row under the General settings **Display** section (next to
  Fullscreen), rendered only when `getProfile().splash` exists — no dead toggle in
  vanilla Coilbox.
- Boot read in `main.tsx`: treat only an explicit `"false"` as off
  (`settingsStorage.get("splash.enabled") !== "false"`), so an unset key still shows.
...
- **Dismiss early**: a click/pointer-down anywhere on the overlay, or pressing
  `Escape`/`Enter`/`Space`, jumps straight to the fade-out phase and then unmounts
  (a quick fade rather than an abrupt cut). Timers are cleared on dismiss and on
  unmount. The overlay is a `<button>`-like focusable surface with an accessible
  label ("Dismiss splash") so the interaction is keyboard-reachable, not mouse-only.
- `prefers-reduced-motion: reduce`: skip the fades; show briefly, then remove.
  Click/key dismiss still removes it immediately.
...
- Live: portable `.coilbox/profile.json` with a `splash` block (local webp and/or
  ...
  fade out over ~2s covering the window, then normal app. Confirm click/Escape
  dismisses early, and the General-settings toggle suppresses it next launch.
